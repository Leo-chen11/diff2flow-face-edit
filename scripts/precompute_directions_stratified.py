...
on the other two binary attributes. All three attributes use 2×2 binary splits.

Glasses  K=4: male×young | male×old | female×young | female×old
Gender   K=4: young×no-glasses | young×glasses | old×no-glasses | old×glasses
Age      K=4: male×no-glasses | male×glasses | female×no-glasses | female×glasses

Predictions must be binary (0/1), as produced by AttributeClassifier.
"""
...
def compute_direction(latents, mask_high, mask_low, min_samples=50):
    """Mean-diff direction from two subsets of latents."""
    ...
    high_mean = latents[mask_high].mean(dim=0)
    low_mean = latents[mask_low].mean(dim=0)
    d = high_mean - low_mean
    if d.norm() < 1e-6:
        return None, n_high, n_low
    return d, n_high, n_low


def _fallback_direction(latents, attr_scores, high_thresh=0.5, low_thresh=0.5):
    """Unconditional mean-diff direction as fallback."""
    d, _, _ = compute_direction(
        latents,
        mask_high=(attr_scores > high_thresh),
        mask_low=(attr_scores <= low_thresh),
    )
    return d
...
def compute_glasses_directions(latents, preds, K=4, min_samples=50):
    """
    Attr 15 (Eyeglasses), conditioned on gender × age.
      K0: male   × young    (gender=1, young=1)
      K1: male   × old      (gender=1, young=0)
      K2: female × young    (gender=0, young=1)
      K3: female × old      (gender=0, young=0)
    """
    glasses = preds[:, 15]
    gender  = preds[:, 20]
    young   = preds[:, 39]
    ...
    for name, sub_mask in strata:
        ...
        sub_gls = glasses[sub_mask]
        d, nh, nl = compute_direction(sub_lat, sub_gls > 0.5, sub_gls <= 0.5, min_samples)
        if d is not None:
            print(f"  glasses/{name}: high={nh}, low={nl}, norm={d.norm():.3f}")
            directions.append(d)
        else:
            print(f"  glasses/{name}: FAILED (high={nh}, low={nl}) → using fallback")
            directions.append(_fallback_direction(latents, glasses))

    return torch.stack(directions[:K])   # (K, 18, 512)


def compute_gender_directions(latents, preds, K=4, min_samples=50):
    """
    Attr 20 (Male), conditioned on age × glasses.
      K0: young × no-glasses  (young=1, glasses=0)
      K1: young × glasses     (young=1, glasses=1)
      K2: old   × no-glasses  (young=0, glasses=0)
      K3: old   × glasses     (young=0, glasses=1)
    """
    gender  = preds[:, 20]
    young   = preds[:, 39]
    glasses = preds[:, 15]
    ...
    for name, sub_mask in strata:
        sub_lat = latents[sub_mask]
        sub_gen = gender[sub_mask]
        d, nh, nl = compute_direction(sub_lat, sub_gen > 0.5, sub_gen <= 0.5, min_samples)
        if d is not None:
            print(f"  gender/{name}: high={nh}, low={nl}, norm={d.norm():.3f}")
            directions.append(d)
        else:
            print(f"  gender/{name}: FAILED (high={nh}, low={nl}) → using fallback")
            directions.append(_fallback_direction(latents, gender))

    return torch.stack(directions[:K])


def compute_age_directions(latents, preds, K=4, min_samples=50):
    """
    Attr 39 (Young), conditioned on gender × glasses.
      K0: male   × no-glasses  (gender=1, glasses=0)
      K1: male   × glasses     (gender=1, glasses=1)
      K2: female × no-glasses  (gender=0, glasses=0)
      K3: female × glasses     (gender=0, glasses=1)
    """
    young   = preds[:, 39]
    gender  = preds[:, 20]
    glasses = preds[:, 15]
    ...
    for name, sub_mask in strata:
        sub_lat = latents[sub_mask]
        sub_yng = young[sub_mask]
        d, nh, nl = compute_direction(sub_lat, sub_yng > 0.5, sub_yng <= 0.5, min_samples)
        if d is not None:
            print(f"  age/{name}: high={nh}, low={nl}, norm={d.norm():.3f}")
            directions.append(d)
        else:
            print(f"  age/{name}: FAILED (high={nh}, low={nl}) → using fallback")
            directions.append(_fallback_direction(latents, young))

    return torch.stack(directions[:K])


def compute_age_k1_stratified(latents, preds, min_samples=50):
    """Debiased K=1 age direction via stratum-size-weighted average.

    Computes 4 sub-directions conditioned on gender × glasses, then
    averages them weighted by stratum size. This removes gender/glasses
    leakage without the norm collapse caused by intra-attr orthogonalization.
    ...
    if not sub_dirs:
        raise ValueError("No valid age sub-directions — check min_samples or data")
...
def main():
    parser = argparse.ArgumentParser(description="Stratified direction bank (binary attrs)")
    ...
    for attr_name, idx in [("glasses", 15), ("gender", 20), ("young", 39)]:
        ...
        print(f"  attr {idx} ({attr_name}): unique values = {uniq}")

    print(f"\nK glasses/gender={K}, K age={age_k}, residual_age={args.residual_age}")

    print(f"\n=== Eyeglasses (attr 15), K={K} ===")
    glasses_dirs = compute_glasses_directions(latents, preds, K, args.min_samples)

    print(f"\n=== Gender / Male (attr 20), K={K} ===")
    gender_dirs  = compute_gender_directions(latents, preds, K, args.min_samples)
    ...
    if age_k == 1:
        age_dir_k1 = compute_age_k1_stratified(latents_for_age, preds, args.min_samples)
        age_dirs   = age_dir_k1.expand(K, -1, -1).clone()   # (K, L, D) — tiled
        skip_age_orth = True
    else:
        age_dirs = compute_age_directions(latents_for_age, preds, age_k, args.min_samples)
        if age_k < K:
