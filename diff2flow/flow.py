...
class FlowModel(nn.Module):
    ...
    def generate(self, x: Tensor, sample_kwargs=None, reverse=False, return_intermediates=False, **kwargs):
        ...
        # ODE sampling
            ...
            # --- [修正開始] ---
            # 移除 kwargs 中已經存在於 cfg_kwargs 的鍵，避免重複傳遞
            if "uc_cond" in kwargs:
                kwargs.pop("uc_cond")
            if "guidance_scale" in kwargs:
                kwargs.pop("guidance_scale")
            if "cond_key" in kwargs:
                kwargs.pop("cond_key")

            # 或者更通用的寫法：
            for k in cfg_kwargs.keys():
                kwargs.pop(k, None)
            # --- [修正結束] ---
            ...
            if method == "euler":
                ...
                for i in tqdm(range(num_steps), disable=not sample_kwargs.get("progress", False), desc="ODE sampling"):
