...
class TrainerModuleLatentFM(LightningModule):
    ...
    def add_lora_to_unet(self):
        ...
        assert not (do_lora_full_attn and (
                    do_lora_self_attn or do_lora_cross_attn)), "LoRA full attn cannot be used with self or cross attn"
        ...
        for path, w in unet.state_dict().items():
            ...
            # Add LoRA layers to the full attention modules
                ...
                # FIX: Explicit rank calculation
                default_rank = math.ceil(
                    max(layer_module.out_features, layer_module.in_features) * lora_cfg_cp.get("lora_full_attn_ratio",
                                                                                               -1))
                rank = lora_cfg_cp.get("lora_full_attn_rank", default_rank)
                if isinstance(rank, bool): rank = lora_cfg_cp.get("lora_rank", 64)
            ...
            # Add LoRA layers to the self attention modules
                ...
                # FIX: Explicit rank calculation
                default_rank = math.ceil(
                    max(layer_module.out_features, layer_module.in_features) * lora_cfg_cp.get("lora_self_attn_ratio",
                                                                                               -1))
                rank = lora_cfg_cp.get("lora_self_attn_rank", default_rank)
                if isinstance(rank, bool): rank = lora_cfg_cp.get("lora_rank", 64)
            ...
            # Add LoRA layers to the cross attention modules
                ...
                # FIX: Explicit rank calculation
                default_rank = math.ceil(
                    max(layer_module.out_features, layer_module.in_features) * lora_cfg_cp.get("lora_cross_attn_ratio",
                                                                                               -1))
                rank = lora_cfg_cp.get("lora_cross_attn_rank", default_rank)
                if isinstance(rank, bool): rank = lora_cfg_cp.get("lora_rank", 64)
            ...
            # Add LoRA layers to the convolutional layers
                ...
                if self.lora_type == "lora":
                    # FIX: Explicit rank calculation
                    default_rank = math.ceil(
                        max(conv_module.in_channels, conv_module.out_channels) * lora_cfg_cp.get("lora_conv_ratio", -1))
                    rank = lora_cfg_cp.get("lora_conv_rank", default_rank)
                    if isinstance(rank, bool): rank = lora_cfg_cp.get("lora_rank", 64)
                    ...
                    ll = LoRAAdapterConv(
                        ...
                        lora_cfg_cp.get("lora_conv_rank",
                                        math.ceil(max(conv_module.in_channels,
                                                      conv_module.out_channels) * lora_cfg_cp.get("lora_conv_ratio",
                                                                                                  -1)),
                                        ),
                        lora_scale,
                    )
            ...
            # Add LoRA layers to the fully connected layers
                      ...
                      ["to_out.0", "proj_in", "proj_out", "ff.net.0.proj", "ff.net.2"]]) and do_lora_mlp:
                print(f"Adding MLP LoRA to {path}")
                fc_module = getattr_recursive(unet, ".".join(path.split(".")[:-1]))

                # FIX: Explicit rank calculation
                default_rank = math.ceil(
                    max(fc_module.out_features, fc_module.in_features) * lora_cfg_cp.get("lora_mlp_ratio", -1))
                rank = lora_cfg_cp.get("lora_mlp_rank", default_rank)
                if isinstance(rank, bool): rank = lora_cfg_cp.get("lora_rank", 64)
        ...
        print(
            f"[LoRA] Optimizing {len(self.params_to_optimize)} LoRA layers and {num_params / 1e6:.2f}M LoRA parameters")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 取得 uncond (空字串的 embedding)
        # 假設你的 cond_stage (OpenCLIP) 有支援 get_unconditional_conditioning
        uc = None
        if hasattr(self, "uncond") and self.uncond is not None:
            uc = self.uncond
        elif hasattr(self.cond_stage, "encode"):
            # 臨時編碼一個空字串
            uc = self.cond_stage.encode([""] * x_source.shape[0])

        generate_kwargs = {
            "context": context,
            "context_ca": conditioning,
            "uc_cond": uc,  # 放這裡
            "guidance_scale": 7.5,  # 放這裡
            "cond_key": "context_ca"  # 放這裡
        }

        x1_latent_pred = model.generate(
            x_source,
            sample_kwargs=sample_kwargs,
            **generate_kwargs  # 用 **kwargs 展開傳入，避免位置參數衝突
        )
        # decode
    ...
    def inference(self, batch, use_ema: bool = True, **kwargs):
        ...
        if "x0_latent" in batch:
            ...
            assert (self.context_key != "x0_latent") & (
                        self.context_key != "x0"), "x0_latent or x0 required for conditioning"
    ...
    def evaluate_and_visualize_batch(self, batch, prefix="train"):
        ...
        generate_kwargs = {
            "context": context,
            "context_ca": conditioning,
            "uc_cond": uc,             # 加入 uncond
            "guidance_scale": 7.5,     # 設定強度 (必須!)
            "cond_key": "context_ca"   # 指定 Key
        }
        ...
        if self.visualizer is not None:
            ...
            if isinstance(self.logger, WandbLogger):
                ...
                try:
                    # 有時候 tensorboard 需要 numpy
                    import numpy as np
                    img = np.array(img)
                    if img.ndim == 3 and img.shape[2] == 3: # HWC to CHW
